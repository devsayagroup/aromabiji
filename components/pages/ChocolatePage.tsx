...
// // GEO ALIGNMENT: Taglines now perfectly match the JSON-LD descriptive tasting notes
// const VARIANTS = [
//   { name: 'Java Preanger',  img: '/products/3d-javapreanger.png',  origin: 'Java, West Java',  slug: 'java-preanger',  tagline: 'Silky caramel. Warm spice.',     color: '#2D5A3D' },
//   { name: 'Alur Badak',     img: '/products/3d-alurbadak.png',     origin: 'North Sumatra',    slug: 'alur-badak',     tagline: 'Earthy depth. Dark fruit finish.', color: '#4A2C6E' },
//   { name: 'Lintong Nihuta', img: '/products/3d-lintongnihuta.png', origin: 'North Sumatra',    slug: 'lintong-nihuta', tagline: 'Deep-earth minerals. Wild berry.',  color: '#1E3A5F' },
//   { name: 'Red Bourbon',    img: '/products/3d-redbourbon.png',    origin: 'South Sumatra',    slug: 'red-bourbon',    tagline: 'Red cherry intensity. Cedar smoke.',color: '#8B1A1A' },
//   { name: 'Toraja',         img: '/products/3d-toraja.png',        origin: 'Sulawesi',         slug: 'toraja',         tagline: 'Bitter cocoa. Wet stone.',         color: '#2C4A1E' },
//   { name: 'Aceh Gayo',      img: '/products/3d-acehgayo.png',      origin: 'Aceh, Sumatra',    slug: 'aceh-gayo',      tagline: 'Honeyed floral. Jasmine lift.',    color: '#D4C5A9' },
//   { name: 'Andung Sari',    img: '/products/3d-andungsari.png',    origin: 'East Java',        slug: 'andung-sari',    tagline: 'White rose. Soft almond.',         color: '#8B7355' },
//   { name: 'Mandailing',     img: '/products/3d-mandailing.png',    origin: 'North Sumatra',    slug: 'mandailing',     tagline: 'Deep cocoa. Exotic sweet spice.',  color: '#C4B99A' },
// ];

// // GEO ALIGNMENT: Features updated to reflect Chocolate & Luwak infusion entities
// const FEATURES = [
//   {
//     number: '01',
//     title: 'Single Origin',
//     body: 'Every dark chocolate bar traces to a specific Indonesian terroir. No blends. Pure single-origin provenance.',
//   },
//   {
//     number: '02',
//     title: 'Wild Luwak Infusion',
//     body: 'Infused with ethically sourced wild Arabica Luwak coffee, naturally fermented in Indonesia’s highland forests.',
//   },
//   {
//     number: '03',
//     title: 'Dark Cacao',
//     body: 'Enough intensity to be honest, with the restraint required to let the regional terroir speak for itself.',
//   },
//   {
//     number: '04',
//     title: 'Artisan Crafted',
//     body: 'Crafted in small batches from bean to bar to preserve the authentic tasting notes of Sumatra, Java, and Sulawesi.',
//   },
// ];
...
// // ══════════════════════════════════════════════════════════════════════════════
// const GAP_VW = 11;
// const FAN_POS = VARIANTS.map((_, i) => -38.5 + i * GAP_VW);

// // ══════════════════════════════════════════════════════════════════════════════
// // HERO SCROLL SECTION
// //
// // 500vh scroll track (up from 400vh).
// // We map the first 80% of the scroll to the animation (matching the old 400vh).
// // The last 20% (100vh) is pure DWELL TIME. The fully revealed collection stays fixed!
// // ══════════════════════════════════════════════════════════════════════════════
// function HeroScroll() {
...
//   const pSpring = useSpring(raw, { stiffness: 70, damping: 22, mass: 1 });

//   // ── THE FIX: CAP THE ANIMATION ─────────────────────────────────────────────
//   // This maps the 0-80% scroll range to the 0-100% animation range.
//   // Anything scrolled past 80% just stays frozen at the final 1.0 animation state.
//   const p = useTransform(pSpring, [0, 0.8], [0, 1]);
...
//                 className="text-pantone/80 text-md md:text-xl text-center font-light max-w-xs md:max-w-3xl mx-auto">
//                 55% single-origin dark chocolate infused with wild Indonesian Luwak Arabica.<br />Uncompromising depth and authentic terroir in every bite.
//             </motion.p>
//         </motion.div>

//         <motion.div
//           style={{ opacity: abtO, x: abtX }}
//           className="absolute right-[25%] md:right-[15%] top-[55%] -mt-[-150px] md:-mt-[110px] w-[min(700px,_70vw)] md:w-[min(400px,_40vw)] pointer-events-none z-20"
//         >
//           <h2
...
//             Pure wild Luwak Arabica from the highlands of Sumatra, Java, and Sulawesi
//             seamlessly fused with 55% dark cacao. Nothing added. Nothing hidden.
//           </p>
//         </motion.div>
...
//   // GEO ALIGNMENT: Swapped coffee terms (Light Roast) for Chocolate and Geo entities
//   const marqueeWords = ['Dark Cacao', '·', 'Wild Luwak Infusion', '·', 'Single Origin', '·', 'Sumatra', '·', 'Java', '·', 'Sulawesi', '·', 'Artisan Crafted', '·', '55% Dark Cacao', '·', 'Wild Luwak Infusion', '·'];
...
//           >
//             Design, heritage, and<br className="hidden md:block" /> absolute Indonesian provenance.
//           </motion.h2>
//         </div>
...
//         >
//           Uncompromising Luxury. That is Aroma Biji.
//         </motion.h2>
...
//             <h3 className="text-3xl md:text-4xl text-canvas font-text font-medium leading-[1.2] max-w-xl mb-8">
//               The Archive. A single origin dark chocolate collection that is truly limited.
//             </h3>
...
// Moved outside to prevent re-declarations on scroll renders
function useIsMobile() {
    const [isMobile, setIsMobile] = useState(false);
    useEffect(() => {
        const mq = window.matchMedia('(max-width: 1024px)');
...
// ── THE FIX: Extracting the Sibling Variant Component ────────────────────────
// This strictly follows React Hook rules. Now useTransform initializes safely 
// once per image instead of re-firing constantly inside a map loop.
function SiblingVariant({ v, si, p, lblO }: { v: any; si: number; p: MotionValue<number>; lblO: MotionValue<number> }) {
  const vi      = si + 1;
  const stagger = si * 0.022;
  const s       = 0.680 + stagger;
  const e       = 0.860 + stagger;

  const x       = useTransform(p, [s, e], [`${FAN_POS[0]}vw`, `${FAN_POS[vi]}vw`]);
  const opacity = useTransform(p, [s, s + 0.05, e], [0, 1, 1]);
  ...
  return (
    <motion.div
      style={{
          ...
          zIndex: si + 1,
          transformOrigin: '50% 50%',
          willChange: 'transform, opacity', // Keeps it hardware accelerated
      }}
    >
      <Image
          src={v.img}
      ...
      >
        <div
          ...
          style={{
            backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 256 256' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)'/%3E%3C/svg%3E")`,
            backgroundSize: '180px',
            transform: 'translateZ(0)', // Force GPU layer for noise background to prevent composite lag
          }}
        />
        ...
        <div
            ...
            {/* Render the rest of the siblings cleanly without breaking Hook Rules */}
            {VARIANTS.slice(1).map((v, si) => (
                <SiblingVariant key={v.name} v={v} si={si} p={p} lblO={lblO} />
            ))}
            </div>
      </div>
    </div>
