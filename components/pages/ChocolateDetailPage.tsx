...
export default function ChocolateDetailClient({
  ...
  const priceCompact = new Intl.NumberFormat("id-ID", {
    style: "currency",
    currency: "IDR",
    ...
    <main className="bg-canvas text-pantone min-h-screen pt-16 md:pt-24 font-text">
        ...
        <div className="w-full lg:w-1/2 p-8 md:p-16 lg:py-20 flex flex-col justify-center">
          ...
          >
            <header className="mb-10">
              ...
              {/* <p className="text-2xl md:text-3xl font-text font-medium text-pantone/90 mb-8">
                {priceCompact}
              </p> */}
      ...
      <section className="bg-canvas border-t border-muted/20">
        <OtherProducts 
          ...
          variant="chocolate" /* <-- THIS IS THE CRITICAL FIX */
        />
      </section>
    </main>
  );
...
function QtyControl({ value, onChange }: { value: number; onChange: (n: number) => void }) {
  return (
    <div className="grid grid-cols-3 items-center border border-muted/50 rounded-full h-14 w-full sm:w-36 bg-white/50 shadow-sm overflow-hidden text-pantone">
      <button 
