...
//   allProducts,
// }: {
//   product: ChocolateProduct & { lottieUrl?: string }; // Added optional lottieUrl type
//   allProducts: ChocolateProduct[];
// }) {
//   const [qty, setQty] = useState(1);
...
//   // Adapter for existing Cart Component (Keeps the static image for the cart thumbnail)
//   const mockVariantForCart = {
//     id: product.id,
//     type: product.weight,
//     price_idr: product.price_idr,
...
//         {/* ─── LEFT SIDE: Lottie Animation Gallery ──────────────────────── */}
//         <div className="relative w-full lg:w-1/2 h-[50vh] lg:h-auto overflow-hidden bg-white/30 ">
//           <AnimatePresence mode="wait">
//             <motion.div
//               key={product.id}
...
//               exit={{ opacity: 0 }}
//               transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
//               className="absolute inset-0 flex items-center justify-end p-0"
//             >
...
//                 // e.g., in your product data: lottieUrl: "/lotties/choco-bar.json"
//                 src={product.lottieUrl || "/lotties/placeholder.json"} 
//                 style={{ height: '100%', width: '100%', maxWidth: '700px', maxHeight: '700px' }}
...
export default function ChocolateDetailClient({
  ...
  // Added videoUrl to your types. Ideally, you want a .webm for Chrome/Firefox and a .mp4 (HEVC) for Safari
  product: ChocolateProduct & { videoUrl?: string; videoUrlSafari?: string }; 
  allProducts: ChocolateProduct[];
}) {
  const [qty, setQty] = useState(1);
  ...
  return (
    <main className="bg-canvas text-pantone min-h-screen pt-16 md:pt-24 font-text">
      <section className="relative flex flex-col lg:flex-row min-h-[85vh]">
        ...
        {/* ─── LEFT SIDE: 360 Video Studio Gallery ──────────────────────── */}
        <div className="relative w-full lg:w-1/2 h-[50vh] lg:h-auto overflow-hidden ">
          
          {/* Luxury Studio Lighting Overlay */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_40%,rgba(255,255,255,0.7)_0%,transparent_70%)] pointer-events-none z-0" />
          <div className="absolute inset-0 bg-canvas pointer-events-none z-0" />
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
              className="absolute inset-0 flex items-center justify-end p-8 md:p-16"
            >
              ...
              <video
                ...
                muted
                playsInline
                className="relative z-10 w-full h-full object-contain max-w-[600px] max-h-[600px] "
              >
                {/* Primary WebM source for Chrome, Edge, Firefox */}
                <source src={product.videoUrl || "/videos/placeholder.webm"} type="video/webm" />
                
                {/* Fallback HEVC MP4 with Alpha for Apple Devices / Safari */}
                <source src={product.videoUrlSafari || "/videos/placeholder.mp4"} type='video/mp4; codecs="hvc1"' />
                
                {/* Final fallback static image if video fails to load on an older browser */}
                <img 
                  src={product.image} 
                  alt={product.name} 
          ...
          <button
            onClick={() => shareProduct(product)}
            className="absolute top-8 left-8 md:top-12 md:left-12 z-20 h-11 w-11 rounded-full border border-ink/10 bg-canvas/60 backdrop-blur-md flex items-center justify-center hover:bg-canvas hover:border-pantone transition-all shadow-sm text-pantone"
            aria-label="Share product"
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                <path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path>
        ...
        {/* ─── RIGHT SIDE: Details ──────────────────────────────────────── */}
        <div className="w-full lg:w-1/2 p-8 md:p-16 lg:py-20 flex flex-col justify-center bg-canvas">
          <motion.div
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
