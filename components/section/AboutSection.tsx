...
export default function AboutSection() {
  ...
  return (
    <section 
      ref={ref} 
      className="relative my-24 mx-6 md:mx-32 lg:mx-[16rem] py-12 md:py-16 bg-ink rounded-4xl overflow-hidden "
    >
      {/* Editorial Background: Using deep --color-ink base with subtle --color-muted glow */}
      <div className="absolute inset-0 bg-[#1A1211]" />
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_85%_35%,rgba(90,71,67,0.15),transparent_60%)]" />
      ...
      <div className="relative z-10 mx-auto max-w-[1300px] px-6 md:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center">
          ...
          <div className="relative">
            <motion.div
              className="relative overflow-hidden rounded-[32px] bg-canvas/5"
            >
              <Image
                src="/images/foto-product-2.jpg"
                alt="Aroma Biji Luwak Coffee"
                width={800}
                height={800}
                className="h-[400px] md:h-[520px] w-full object-cover opacity-90 grayscale-[20%] contrast-[105%]"
              />
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-ink/60 to-transparent" />
            </motion.div>
          </div>

          {/* CONTENT: Typography prioritized with the new color palette */}
          <motion.div
            style={{ y: textY }}
            initial={{ opacity: 0, y: 18 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            >
              <span className="text-[10px] tracking-[0.4em] uppercase text-canvas/50 font-medium">
                Our Story
              </span>
            </motion.div>

            <h2 className="font-style uppercase tracking-wider leading-[1.1] text-3xl md:text-5xl text-canvas">
              Crafted Through Generations
            </h2>

            <p className="mt-8 font-text text-sm md:text-base leading-[1.9] text-canvas/70 max-w-xl font-light">
              For over 40 years, we’ve dedicated our hearts to the art of coffee making. From humble
              beginnings to a legacy of craftsmanship, Aroma Biji stands as a symbol of authenticity.
              Our journey is guided by generations who believe great coffee comes from respect for
              nature, for people, and for tradition.
